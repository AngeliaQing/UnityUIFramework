...
{
    SortedDictionary<string, KeyValuePair<GameObject, UIBase>> m_loaded_prefab = new SortedDictionary<string, KeyValuePair<GameObject, UIBase>>();
    public void LoadUI(string ui_name, Action<UIBase> on_load, string ui_dir_path = "")
    {
        ...
        Debug.LogWarning("UIPrefabMgr LoadUI, UI/" + ui_name);
        go.name = ui_name;  // AssetBundle的名字 == 界面GameObject的名字 == 脚本的名字
        Transform tf = go.transform;
        tf.parent = UIBase.GetUIParent();
        tf.localScale = Vector3.one;
        tf.localPosition = Vector3.zero;
        m_loaded_prefab[ui_name] = new KeyValuePair<GameObject, UIBase>(go, ui_base);
    ...
    public UIBase GetLoadedUI(string ui_name)
    {
        KeyValuePair<GameObject, UIBase> pair;
        if (m_loaded_prefab.TryGetValue(ui_name, out pair))
            return pair.Value;
    ...
    public void DestroyUI(string ui_name)
    {
        ...
        Debug.LogWarning("UIPrefabMgr DestroyUI, UI/Layout/" + ui_name);
    }
