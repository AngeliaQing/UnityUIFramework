...
class InputManager : SingletonMonoBehaviour<InputManager>
{
	...
	private List<InputHandler> m_input_listener_list = new List<InputHandler>();

    //public static InputManager Instance { private set; get; }
    void Awake()
	{
        //Instance = this;
        GameObject each_touch_obj = GameObject.Find("GameGlobal/InputManager");
        if (each_touch_obj == null)
        {
            Debug.LogError("InputManager, GameGlobal/InputManager is null");
            return;
        }
        m_easy_touch = each_touch_obj.GetComponent<EasyTouch>();
        ...
        GameObject UICamera1 = UIHelper.GetUICamera();
        if (UICamera1 == null)
            Debug.LogError("InputManager, UICamera is null");
        else
            AddInterceptiveCamera(UICamera1.GetComponent<Camera>());
        //GameObject UICamera2 = GameObject.Find("GUI/UICameraTopmost");
        //if (UICamera2 == null)
        //    LogWrapper.LogError("InputManager, GUI/UICameraTopmost is null");
        //else 
        //    AddInterceptiveCamera(UICamera2.GetComponent<Camera>());
    ...
    protected override void OnDestroy()
    {
        ...
        if (m_easy_touch.gameObject != null)
            Object.Destroy(m_easy_touch.gameObject);
    }
